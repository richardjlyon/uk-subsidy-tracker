...
- **D-11 Mutualisation rolled into `ro_cost_gbp`; affected year flagged.** `roc_price_gbp` for obligation year **2021-22 only** is adjusted upward by the published mutualisation delta. Research (05-RESEARCH.md §4) verified that only SY 2021-22 triggered mutualisation: £119.7M shortfall, £44.0M redistributed by Ofgem in December 2022. SY 2022-23 fell below threshold (£7.2M shortfall vs ~£318M mutualisation ceiling — no trigger); mutualisation_gbp is null from 2022-23 onward. `annual_summary.parquet` carries an additional `mutualisation_gbp` column showing the additive component on 2021-22 only (null otherwise). S2 dynamics chart visibly spikes on the single affected year; methodology section documents the event ("Suppliers defaulted on their ROC obligations in the 2021 energy-price crisis; Ofgem redistributed the shortfall across remaining suppliers, who passed the cost to consumers"). Mutualisation is included in REF-benchmark reconciliation (D-14); if REF excluded mutualisation, that's a scope-delta entry in benchmarks.yaml audit header.
...
- **D-13 REF Constable 2025 anchor (supersedes Turver).** Research (05-RESEARCH.md §5) identified **REF (Renewable Energy Foundation) Constable 2025-05-01 "Renewable Subsidies" Table 1** (`ref.org.uk/attachments/article/390/renewables.subsidies.01.05.25.pdf`) as the primary RO benchmark anchor. REF publishes a full transcribable 22-year RO cost series (£67.0bn cumulative through 2023/24); Turver's Net Zero Watch material only exposes 2-3 single-point figures and is insufficient as a 2011-2022 anchor. Benchmark entry: `tests/fixtures/benchmarks.yaml::ref_constable` (NOT `turver`) gets a new top-level key with entries per transcribed year per RESEARCH §5's verbatim transcription. Tolerance constant: `REF_TOLERANCE_PCT = 3.0` in `tests/fixtures/__init__.py` or equivalent `_TOLERANCE_BY_SOURCE` dispatch per Phase 2 D-07. Audit-header note in benchmarks.yaml cites REF publication + retrieval date + page reference. Phase 2 D-11 fallback (skip-if-unavailable) **does not apply** here — ROADMAP SC #3 is binary. (Turver substack may still be cited in `docs/schemes/ro.md` methodology section as a peer cross-check, but is not the test anchor.)
- **D-14 ±3% divergence is a hard block.** `test_benchmarks.py` parametrises over the REF Constable anchor years; aggregate `ro_cost_gbp` (all-in GB total per D-12 scope) must be within ±3% of REF's per-year figures (Table 1). If divergence exceeds 3%, investigate root cause before phase exit:
  - Is REF's scope different? (NIRO inclusion, mutualisation treatment, cofiring attribution, CY vs OY) → document scope-delta in benchmarks.yaml audit header; adjust tolerance only if scope difference is documented and legitimate.
...
- **D-15 `docs/schemes/ro.md` single-page scheme overview.** Structure:
  ...
  7. **Methodology summary** — link to `docs/methodology/gas-counterfactual.md` + co-firing/mutualisation/NIRO caveats + REF Constable 2025 benchmark reconciliation (with optional note citing Turver Net Zero Watch as peer cross-check).
...
- **Ofgem scraper mechanism — attempt programmatic RER.** Ofgem's Renewables & CHP Register was replaced 2025-05-14 by the new Renewables Energy Register (RER) at `rer.ofgem.gov.uk`; public data access is now SharePoint + email-auth-required (research 05-RESEARCH.md §1, LOW-MEDIUM confidence). **User decision (2026-04-22): planner attempts programmatic scraping** rather than falling back to manual-refresh (Option D rejected). Planner investigates at plan-time: (a) Microsoft Graph API via SharePoint app registration, (b) Playwright with email-auth flow, (c) direct XLSX/CSV download URLs if Ofgem exposes them on the RER dashboard. If all three fail, planner must escalate back to the user before committing to Option D — do not silently regress to manual-refresh. Error-path discipline follows Phase 4 Plan 07 pattern (output_path bound before try, timeout=60, bare raise on failure). Fallback raw-file snapshot may be committed to `data/raw/ofgem/` as a one-time seed so the pipeline is not blocked while RER auth is being plumbed (planner decides whether seed commit is necessary).
...
- `tests/fixtures/benchmarks.yaml` — Add `ref_constable` section with CY 2011-2022 annual totals transcribed from REF Constable 2025-05-01 Table 1 (verbatim in 05-RESEARCH.md §5) + audit-header note citing publication + retrieval date.
...
- `tests/test_benchmarks.py` — Add REF Constable reconciliation test over RO aggregate with `REF_TOLERANCE_PCT = 3.0` (source key `ref_constable` in `_TOLERANCE_BY_SOURCE` dispatch).
...
- **REF (Renewable Energy Foundation) "Renewable Subsidies" Constable 2025-05-01 Table 1** — https://ref.org.uk/attachments/article/390/renewables.subsidies.01.05.25.pdf — **primary RO-06 benchmark anchor (confirmed by 05-RESEARCH.md §5)**. 22-year RO cost series; 2011-2022 entries transcribed into `benchmarks.yaml::ref_constable`.
- Turver 2023 Net Zero Watch paper "The Hidden Cost of Net Zero" — secondary/peer cross-check only; superseded as primary anchor by REF Constable 2025.
...
- **`tests/fixtures/benchmarks.yaml`** + `tests/fixtures/__init__.py::load_benchmarks` + `_TOLERANCE_BY_SOURCE` dispatch — Pattern for adding REF Constable section (`ref_constable`) with `REF_TOLERANCE_PCT = 3.0` constant.
...
- **Two-layer Pydantic + source-key injection** (Phase 2 D-07). `ro_bandings.yaml` + `RoBandingEntry` + `RoBandingTable`; `benchmarks.yaml::ref_constable` + existing `BenchmarkEntry` model (source key injected for tolerance dispatch).
...
- **"REF Constable defines the acceptance test"** — RO-06's 3% tolerance is our public commitment to numerical accuracy. If we can't replicate REF Constable 2025 Table 1 per-year totals within 3%, the project's core value ("every headline number reproducible") is unproven for RO. Hence D-14's hard block. Turver Net Zero Watch remains a valuable peer cross-check but is not the binary test anchor.
...
## Post-Research Amendments (2026-04-22)

Applied after `gsd-phase-researcher` surfaced three material findings in `05-RESEARCH.md`. User-confirmed via plan-phase gate questions.

1. **D-13/D-14 anchor switch.** Turver Net Zero Watch substack replaced by **REF (Renewable Energy Foundation) Constable 2025-05-01 Table 1** as the primary 2011-2022 benchmark anchor. REF publishes a full 12-year transcribable series; Turver only exposes 2-3 single-point figures. Section key renamed `turver` → `ref_constable`; tolerance constant renamed `TURVER_TOLERANCE_PCT` → `REF_TOLERANCE_PCT = 3.0`. Turver retained as peer cross-check in methodology page prose. Evidence in 05-RESEARCH.md §5.
2. **D-11 mutualisation scope correction.** Research verified only **SY 2021-22** triggered mutualisation (£119.7M shortfall → £44.0M redistribution Dec 2022). SY 2022-23 fell below threshold (£7.2M vs £318M ceiling — no trigger). `mutualisation_gbp` column is non-null for 2021-22 only; S2 chart spikes on a single year. Evidence in 05-RESEARCH.md §4.
3. **Ofgem scraper posture — attempt programmatic RER.** Ofgem's legacy `renewablesandchp.ofgem.gov.uk` was replaced 2025-05-14 by `rer.ofgem.gov.uk` (SharePoint + email auth). User rejected the researcher's Option D manual-refresh recommendation; planner must investigate programmatic options (MS Graph API, Playwright email-auth, direct RER dashboard URLs) at plan-time and escalate back to user before falling back to manual-refresh. Evidence in 05-RESEARCH.md §1.

---

*Phase: 05-ro-module*
*Context gathered: 2026-04-22*
*Amended: 2026-04-22 post-research*
